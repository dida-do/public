...
class PPVThresholds(nn.Module):
    """Threshold layer for MiniRocket transformation of timeseries.
    ...
    def _select_quantiles(
        ...
        Alternatively, quantiles are uniformly spaced in [0, 1].
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if not self.is_fitted:
            self.fit(x)
            logger.warning("automatically fit biases to first training example")
